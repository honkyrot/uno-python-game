...
class Game:
    ...
    def next_plr_turn(self, plr: deck.Player, amt: int) -> None:
        players = [self.Player, self.CPU1, self.CPU2, self.CPU3]
        curr_player_idx = players.index(plr)
        if self.table_clockwise:
            next_player_idx = (curr_player_idx + amt) % len(players)
        else:
            next_player_idx = (curr_player_idx - amt) % len(players)
        self.current_player = players[next_player_idx]
        print(self.current_player.user_name + "'s turn.")

    def skip(self, plr: deck.Player) -> None:
        self.next_plr_turn(plr, 2)

    def reverse(self, plr: deck.Player) -> None:
        if self.table_clockwise:
    ...
    def wild(self, plr: deck.Player, color: str) -> None:
        self.table_color = color
        self.next_plr_turn(plr, 1)

    def draw_two(self, plr: deck.Player) -> None:
        if self.table_clockwise:
            if plr == self.Player:
    ...
    def draw_four(self, plr: deck.Player, color: str) -> None:
    ...
    def gameplay(self) -> None:
        ...
        if self.current_player == self.Player:
            ...
            if choice == "1":
                ...
                print(f"{self.current_player.user_name} has chosen to draw a card.")
        ...
        if self.current_player != self.Player:
            print(f"It's {self.current_player.user_name}'s turn.")
            time.sleep(3)
            for i in range(0, len(self.current_player.hand)):
                ...
                if color == self.table_color or value == self.table_value:
                    ...
                    print(f"{self.current_player.user_name} has chosen to play {card}")
                    self.next_plr_turn(self.current_player, 1)
                    self.gameplay()

                elif self.current_player.hand[i].value == "reverse":
                    print(f"{self.current_player.user_name} played {color} {value}")
                    self.reverse(self.current_player)
                    self.gameplay()
